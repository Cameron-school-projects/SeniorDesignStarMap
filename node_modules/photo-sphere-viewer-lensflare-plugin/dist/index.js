(function (global, factory) {
	...
	class LensflareObject {
	    ...
	    constructor(viewer, config) {
	        ...
	        this.refreshAfterLoad = (texture) => {
	            texture.anisotropy = 4;
	            this.viewer.needsUpdate();
	        };
	        if (!config.id) {
	            throw new core.PSVError('missing lensflare id');
	        }
	    ...
	    addLight(h, s, l, x, y, z) {
	        ...
	        switch (this.config.type) {
	            ...
	            default:
	                ...
	                lensflare.addElement(new LensflareElement(this.textureFlare["diaphragm"], 130, 0.45, new three.Color(0x02fa12)));
	                lensflare.addElement(new LensflareElement(this.textureFlare["diaphragm"], 110, 0.55, new three.Color(0x00ff12)));
	                lensflare.addElement(new LensflareElement(this.textureFlare["flare3"], 70, 0.7, new three.Color(0x0223a3)));
	                lensflare.addElement(new LensflareElement(this.textureFlare["flare3"], 80, 0.8, new three.Color(0x02fa12)));
	     ...
	     */
	    update(config) {
	        if (core.utils.isExtendedPosition(config)) {
	            core.utils.logWarn('Use the "position" property to configure the position of a lensflare');
	            config.position = this.viewer.dataHelper.cleanPosition(config);
	        }
	        if ('width' in config && 'height' in config) {
	            core.utils.logWarn('Use the "size" property to configure the size of a lensflare');
	            // @ts-ignore
	            config.size = { width: config['width'], height: config['height'] };
	        }
	        this.config = core.utils.deepmerge(this.config, config);
	        ...
	        this.state.anchor = core.utils.parsePoint(this.config.anchor);
	        this.__update3d();
	    }
	    /**
	    ...
	    __update3d() {
	        ...
	        if (!core.utils.isExtendedPosition(this.config.position)) {
	            throw new core.PSVError('missing lensflare position');
	        }
	    ...
	    __createMesh() {
	        ...
	        const element = new three.Group().add(mesh).add(light);
	 ...
	 */
	class LensflaresPluginEvent extends core.TypedEvent {
	}
	/**
	 * @event Triggered when the visibility of a lensflare changes
	 */
	class LensflareVisibilityEvent extends LensflaresPluginEvent {
	    constructor(lensflare, visible) {
	 ...
	 */
	class SetLensflaresEvent extends LensflaresPluginEvent {
	    constructor(lensflares) {
	...
	const getConfig = core.utils.getConfigParser({
	    lensflares: null,
	});
	class LensflarePlugin extends core.AbstractPlugin {
	    ...
	    init() {
	        super.init();
	        this.viewer.addEventListener(core.events.RenderEvent.type, this);
	        this.viewer.addEventListener(core.events.ReadyEvent.type, this, { once: true });
	    }
	    destroy() {
	        // do your cleanup logic here
	        this.clearLensflares(false);
	        this.viewer.removeEventListener(core.events.RenderEvent.type, this);
	        this.viewer.removeEventListener(core.events.ReadyEvent.type, this);
	        super.destroy();
	    }
	    /**
	    * @internal
	    */
	     ...
	     */
	    addLensflare(config, render = true) {
	        if (this.lensflares[config.id]) {
	            throw new core.PSVError(`lensflare "${config.id}" already exists`);
	        }
	     ...
	     */
	    updateLensflare(config, render = true) {
	     ...
	     */
	    removeLensflare(lensflareId, render = true) {
	     ...
	     */
	    removeLensflares(lensflareIds, render = true) {
	     ...
	     */
	    setLensflares(lensflares, render = true) {
	        this.clearLensflares(false);
